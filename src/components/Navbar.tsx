            ...
            <div className="hidden md:flex items-center gap-2">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className={`px-4 py-2 text-base font-semibold transition-all duration-300 rounded focus:outline-none focus:ring-2 focus:ring-brand-yellow/70 relative group overflow-hidden ${
                ...
                >
                  ...
                  <span className={`absolute left-0 bottom-0 h-0.5 bg-brand-yellow transition-all duration-500 rounded-full ${
            ...
            <div className="flex flex-col items-center gap-2 p-2">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className={`block px-5 py-3 text-base font-semibold w-full text-center rounded-lg transition-all duration-300 ${
                    activeSection === link.href.substring(1)
